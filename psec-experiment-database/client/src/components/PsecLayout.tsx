      ...
      <header className="navy-grid glass-header sticky top-0 z-30 border-b border-white/10 text-white">
        <div className="mx-auto flex min-h-[72px] max-w-[1440px] items-center justify-between gap-4 px-5 py-3 lg:px-10">
          ...
          <div className="flex items-center gap-2">
            <Link href="/#archive-search" className="focus-ring hidden items-center gap-2 rounded-full px-3 py-2 text-sm text-white/68 transition-colors hover:bg-white/10 hover:text-white md:flex"><Search size={14} /> {t("search")}</Link>
            ...
            <button type="button" aria-label={t("languageLabel")} onClick={toggleLanguage} className="focus-ring language-toggle hidden items-center gap-2 border border-white/20 px-3 py-2 text-sm text-white/75 transition-colors hover:border-white/45 hover:bg-white/10 hover:text-white sm:flex"><Languages size={14} /><span>{language === "zh" ? "EN" : "中"}</span></button>
            <button aria-label={t("menu")} aria-expanded={mobileOpen} className="focus-ring flex h-10 w-10 items-center justify-center rounded-full border border-white/20 text-white transition-colors hover:bg-white/10 lg:hidden" onClick={() => setMobileOpen((value) => !value)}>{mobileOpen ? <X size={19} /> : <Menu size={19} />}</button>
          </div>
        </div>
        {mobileOpen && <nav className="border-t border-white/10 px-5 py-4 lg:hidden" aria-label={t("mobileNavigation")}>
          <div className="mx-auto grid max-w-2xl gap-1">
            ...
            <Link href="/#archive-search" onClick={() => setMobileOpen(false)} className="focus-ring flex items-center gap-2 rounded-lg px-3 py-3 text-sm text-white/75 transition-colors hover:bg-white/10 hover:text-signal"><Search size={15} /> {t("search")}</Link>
