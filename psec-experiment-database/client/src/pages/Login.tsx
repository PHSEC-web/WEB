    ...
    <div className="mx-auto grid max-w-5xl gap-12 lg:grid-cols-[1fr_420px] lg:items-center">
      ...
      <div className="border border-border bg-card p-7 md:p-9">
        ...
        {!sent ? <form onSubmit={submitEmail} className="mt-7">
          <h2 className="font-display text-2xl">{t("getSignInCode")}</h2>
          <label className="mt-6 block"><span className="form-label">{t("schoolEmail")}</span><input autoFocus type="email" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder={`name${schoolDomain}`} className="form-control mt-2" /></label>
          <button type="submit" disabled={requestCode.isPending} className="focus-ring mt-5 flex h-12 w-full items-center justify-center gap-2 bg-primary font-mono text-[10px] uppercase tracking-[.14em] text-white hover:bg-[#083d80]">{requestCode.isPending ? t("sendingCode") : t("sendSignInCode")}<ArrowRight size={15} /></button>
        </form> : <form onSubmit={submitCode} className="mt-7">
          <h2 className="font-display text-2xl">{t("enterYourCode")}</h2>
          <p className="mt-3 text-sm leading-6 text-muted-foreground">{t("checkEmail")} <strong className="font-medium text-ink">{email}</strong>. {t("codeExpires")}</p>
          <label className="mt-6 block"><span className="form-label">{t("sixDigitCode")}</span><input autoFocus inputMode="numeric" pattern="[0-9]{6}" maxLength={6} required value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, ""))} placeholder="000000" className="form-control mt-2 font-mono tracking-[.4em]" /></label>
          <button type="submit" disabled={verifyCode.isPending} className="focus-ring mt-5 flex h-12 w-full items-center justify-center gap-2 bg-primary font-mono text-[10px] uppercase tracking-[.14em] text-white hover:bg-[#083d80]">{verifyCode.isPending ? t("verifying") : t("verifyContinue")}<ArrowRight size={15} /></button>
          <button type="button" onClick={() => { setSent(false); setCode(""); setMessage(""); }} className="mt-4 w-full text-center font-mono text-[10px] uppercase tracking-[.14em] text-muted-foreground hover:text-primary">{t("useDifferentEmail")}</button>
        </form>}
        ...
        <Link href="/" className="mt-7 block text-center font-mono text-[10px] uppercase tracking-[.14em] text-muted-foreground hover:text-primary">{t("returnToArchive")}</Link>
      </div>
    </div>
  </div>;
}
