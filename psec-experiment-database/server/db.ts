...
export async function listCompletedExperiments() {
  ...
  return db
    ...
    .where(
      and(
        eq(records.recordKind, "project"),
        eq(records.status, "published"),
        eq(records.lifecycle, "completed"),
...
export async function listExecutionRecords(slug: string) {
  ...
  const legacy = record
    ...
    : (
        await db
          .select({ id: experiments.id, category: experiments.category })
          .from(experiments)
  ...
  if (
    !record &&
    (!legacy || legacy.category !== "Completed Experimental Projects")
  )
    return [];
  const rows = record
    ? await db
        ...
        .where(eq(executionRecords.recordId, record.id))
        .orderBy(desc(executionRecords.createdAt))
    : await db
        .select()
        .from(executionRecords)
        .where(eq(executionRecords.experimentId, legacy!.id))
        .orderBy(desc(executionRecords.createdAt));
  return Promise.all(
    rows.map(async row => ({
      ...row,
      attachments: (
        await db
          ...
          .where(
            and(
              eq(attachments.executionRecordId, row.id),
...
export async function createEvidenceSubmission(input: {
  ...
  for (const file of input.files ?? []) {
    ...
    await db.insert(attachments).values({
      ...
      kind: stored.kind,
      visibility: "members",
      uploadedByName: input.submitterName.trim(),
    });
  }
  return { id };
}
...
export async function listPendingEvidenceSubmissions() {
  ...
  const rows = await db
    .select({
      ...
      recordCategory: records.category,
      legacyTitle: experiments.title,
      legacySlug: experiments.slug,
      legacyCategory: experiments.category,
    })
    .from(evidenceSubmissions)
    .leftJoin(
      records,
      and(
        eq(evidenceSubmissions.recordId, records.id),
        isNull(records.deletedAt)
      )
    )
    .leftJoin(experiments, eq(evidenceSubmissions.experimentId, experiments.id))
    .where(eq(evidenceSubmissions.status, "pending"))
    .orderBy(desc(evidenceSubmissions.submittedAt));
  return Promise.all(
    rows.map(async row => ({
      ...row.evidence,
      experimentTitle: row.recordTitle || row.legacyTitle,
      experimentSlug: row.recordSlug || row.legacySlug,
      experimentCategory: row.recordCategory || row.legacyCategory,
      attachments: (
        await db
          .select({
            id: attachments.id,
          ...
          .where(
            and(
              eq(attachments.evidenceSubmissionId, row.evidence.id),
              isNull(attachments.deletedAt)
            )
          )
          .orderBy(desc(attachments.createdAt))
...
export async function approveEvidenceSubmission(input: {
  id: number;
  editor: string;
}) {
  const db = await getDb();
  ...
  const current = (
    await db
      .select()
      .from(evidenceSubmissions)
      .where(eq(evidenceSubmissions.id, input.id))
      .limit(1)
  )[0];
  if (!current || current.status !== "pending")
    throw new Error("Evidence submission is no longer pending");
  if (current.recordId) {
    const target = (
      await db
        .select({ id: records.id })
        ...
        .where(
          and(
            eq(records.id, current.recordId),
            eq(records.recordKind, "project"),
            eq(records.status, "published"),
            eq(records.lifecycle, "completed"),
            isNull(records.deletedAt)
    ...
    if (!target) {
      await db
        ...
        .set({
          status: "rejected",
          rejectionComment:
            "The selected completed project is no longer available.",
          reviewedAt: new Date(),
          reviewedBy: input.editor,
        })
        .where(eq(evidenceSubmissions.id, input.id));
      throw new Error(
        "The selected completed project is no longer available; the evidence was rejected and retained."
      );
    }
    const execution = await db.insert(executionRecords).values({
      experimentId: null,
      recordId: target.id,
      evidenceSubmissionId: current.id,
      submitterName: current.submitterName,
      observationNotes: current.observationNotes,
    });
    ...
    await db
      ...
      .where(eq(attachments.evidenceSubmissionId, current.id));
    await db
      .update(evidenceSubmissions)
      .set({
        status: "approved",
      ...
      .where(eq(evidenceSubmissions.id, input.id));
    return { id: current.id, recordId: target.id, executionRecordId };
  }
  if (current.experimentId === null)
    throw new Error("Evidence target is missing");
  const target = (
    await db
      .select({ id: experiments.id, category: experiments.category })
      .from(experiments)
      .where(eq(experiments.id, current.experimentId))
      .limit(1)
  )[0];
  if (!target || target.category !== "Completed Experimental Projects") {
    await db
      ...
      .set({
        status: "rejected",
        rejectionComment:
          "The selected completed experiment is no longer available.",
        reviewedAt: new Date(),
        reviewedBy: input.editor,
      })
      .where(eq(evidenceSubmissions.id, input.id));
    throw new Error(
      "The selected completed experiment is no longer available; the evidence was rejected and retained."
    );
  }
  const execution = await db.insert(executionRecords).values({
  ...
  await db
    .update(attachments)
    .set({ experimentId: target.id, executionRecordId, visibility: "public" })
    .where(eq(attachments.evidenceSubmissionId, current.id));
  await db
    .update(evidenceSubmissions)
    .set({
...
export async function rejectEvidenceSubmission(input: {
  id: number;
  editor: string;
  comment: string;
}) {
  ...
  await db
    ...
    .set({
      status: "rejected",
      rejectionComment: input.comment.trim(),
      reviewedAt: new Date(),
      reviewedBy: input.editor,
    })
    .where(eq(evidenceSubmissions.id, input.id));
...
export async function deleteExperimentAsAdmin(
  id: number,
  confirmTitle: string
) {
  const db = await getDb();
...
export async function listRecentSubmissions(limit = 5) {
  ...
  const rows = await db
    .select({
      ...
      discipline: submissions.publishedDiscipline,
      category: submissions.publishedCategory,
...
async function storeAttachment(
  ...
  const safeName =
    input.fileName.replace(/[^a-zA-Z0-9._-]/g, "-").slice(0, 180) ||
    "attachment";
...
export async function listPendingSubmissions(filters?: {
  discipline?: string;
  from?: number;
  to?: number;
}) {
    ...
    filtered.map(async row => ({
      ...
      attachments: (
        await db
          ...
          .where(
            and(
              eq(attachments.submissionId, row.id),
              isNull(attachments.deletedAt)
            )
          )
          .orderBy(desc(attachments.createdAt))
...
export async function listExperimentAttachments(slug: string) {
  ...
  if (record) {
    const rows = await db
      ...
      .where(
        and(
          eq(attachments.recordId, record.id),
          eq(attachments.visibility, "public"),
          isNull(attachments.deletedAt)
        )
      )
  ...
  if (!experiment || experiment.category !== "Completed Experimental Projects")
    return [];
  const rows = await db
    .select({
      id: attachments.id,
    ...
    .where(
      and(
        eq(attachments.experimentId, experiment.id),
        eq(attachments.visibility, "public"),
        isNull(attachments.deletedAt)
      )
    )
...
export async function uploadExperimentAttachments(input: {
  experimentId: number;
  files: AttachmentInput[];
  uploader: string;
}) {
  ...
  const record = (
    await db
      ...
      .where(
        and(
          eq(records.id, input.experimentId),
          eq(records.recordKind, "project"),
          eq(records.status, "published"),
          isNull(records.deletedAt)
        )
  ...
  if (!record || record.lifecycle !== "completed")
    throw new Error("Choose an available completed project");
  for (const file of input.files) {
    const stored = await storeAttachment(file, `psec-records/${record.id}`);
    await db.insert(attachments).values({
      recordId: record.id,
      experimentId: null,
      submissionId: null,
  ...
  return { experiment: record, count: input.files.length };
}
...
export async function updateSubmissionAsAdmin(input: {
  id: number;
  editor: string;
  ...
  if (input.values.attachmentData) {
    const safeName = (input.values.attachmentName || "attachment").replace(
      /[^a-zA-Z0-9._-]/g,
      "-"
    );
    const base64 = input.values.attachmentData.includes(",")
      ? input.values.attachmentData.split(",").pop()
      : input.values.attachmentData;
    replacement = await storagePut(
      `psec-submissions/${input.id}/iterations/${Date.now()}-${safeName}`,
      Buffer.from(base64 || "", "base64"),
      input.values.attachmentMimeType || "application/octet-stream"
    );
  }
  const next = {
    ...
    attachmentName: replacement
      ? input.values.attachmentName || current.attachmentName
  ...
  await db.insert(submissionHistory).values({
    ...
    snapshotJson: JSON.stringify({
      before: submissionSnapshot(current),
      after: submissionSnapshot(next),
    }),
    createdAt: now,
  });
  return { id: input.id };
...
export async function approveAndArchiveSubmission(input: {
  id: number;
  editor: string;
  discipline: string;
  category: string;
  ...
  const experimentResult = await db.insert(experiments).values({
    ...
    theoreticalBasis:
      current.theoreticalBasis ||
      "Member-submitted idea; theoretical basis not yet recorded.",
    historicalBackground:
      current.historicalBackground ||
      `Submitted by ${current.memberName} (${current.memberId}).`,
  ...
  await db
    .update(attachments)
    .set({ experimentId, visibility: "public" })
    .where(eq(attachments.submissionId, current.id));
  await db
    .update(submissions)
    .set({
      status: "approved",
      publishedDiscipline: input.discipline,
      publishedCategory: input.category,
      reviewedAt: now,
      reviewedBy: input.editor,
      updatedAt: now,
    })
