...
export const records = mysqlTable("records", {
  id: int("id").autoincrement().primaryKey(),
  slug: varchar("slug", { length: 160 }).notNull().unique(),
  recordKind: mysqlEnum("recordKind", ["reference", "project"]).notNull().default("project"),
  discipline: varchar("discipline", { length: 80 }).notNull(),
  category: varchar("category", { length: 100 }).notNull(),
  status: mysqlEnum("status", ["pending", "needs_revision", "published", "rejected", "hidden"]).notNull().default("pending"),
  lifecycle: mysqlEnum("lifecycle", ["idea", "design", "in_progress", "completed"]),
  visibility: mysqlEnum("visibility", ["public", "members"]).notNull().default("public"),
  title: varchar("title", { length: 240 }).notNull(),
  ...
  disciplineCategoryStatusIdx: index("records_discipline_category_status_idx").on(table.discipline, table.category, table.status),
  kindStatusPublishedIdx: index("records_kind_status_published_idx").on(table.recordKind, table.status, table.publishedAt),
  ownerIdx: index("records_owner_idx").on(table.ownerOpenId),
  forkIdx: index("records_fork_idx").on(table.forkOfRecordId),
}));

export const recordRevisions = mysqlTable("recordRevisions", {
  id: int("id").autoincrement().primaryKey(),
  ...
  editorRole: mysqlEnum("editorRole", ["member", "admin", "system"]).notNull().default("system"),
  note: text("note"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({
  recordRevisionUnique: uniqueIndex("record_revisions_record_revision_unique").on(table.recordId, table.revisionNo),
  recordCreatedIdx: index("record_revisions_record_created_idx").on(table.recordId, table.createdAt),
}));
...
export const attachments = mysqlTable("attachments", {
  ...
  kind: mysqlEnum("kind", ["report", "protocol", "data", "photo", "other"]).notNull().default("other"),
  visibility: mysqlEnum("visibility", ["public", "members"]).notNull().default("members"),
  uploadedByOpenId: varchar("uploadedByOpenId", { length: 64 }),
  uploadedByName: varchar("uploadedByName", { length: 160 }),
  deletedAt: timestamp("deletedAt"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({
  recordCreatedIdx: index("attachments_record_created_idx").on(table.recordId, table.createdAt),
  experimentCreatedIdx: index("attachments_experiment_created_idx").on(table.experimentId, table.createdAt),
  submissionCreatedIdx: index("attachments_submission_created_idx").on(table.submissionId, table.createdAt),
  evidenceSubmissionCreatedIdx: index("attachments_evidence_submission_created_idx").on(table.evidenceSubmissionId, table.createdAt),
  executionRecordCreatedIdx: index("attachments_execution_record_created_idx").on(table.executionRecordId, table.createdAt),
}));
...
export const executionRecords = mysqlTable("executionRecords", {
  id: int("id").autoincrement().primaryKey(),
  experimentId: int("experimentId").notNull(),
  evidenceSubmissionId: int("evidenceSubmissionId"),
  ...
  experimentCreatedIdx: index("execution_records_experiment_created_idx").on(table.experimentId, table.createdAt),
}));
